...
export class Guest_BooksService {
  ...
  async getOneById(id: number) {
    const guestBook = await this.guest_bookRepository.findOne({
      where: { id },
    });
    return guestBook;
  }
  ...
  async create(
    createGuestBookRequest: CreateGuestBookDto,
  ): Promise<Guest_Book> {
    const newDiary = this.guest_bookRepository.create({
  ...
  async update(id: number, updateGuestBookRequest: UpdateGuestBookDto) {
    const toUpdate = await this.guest_bookRepository.findOne({
      where: { id: id },
    });
    if (!toUpdate) {
    ...
    const isValidPassword =
      updateGuestBookRequest.password === toUpdate.password;
    if (!isValidPassword) {
  ...
  async delete(id: number): Promise<void> {
    const toDelete = await this.guest_bookRepository.findOne({
      where: { id: id },
    });
  ...
  async getHided(): Promise<Guest_Book[]> {
    const hided = this.guest_bookRepository.find({

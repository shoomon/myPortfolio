...
export class SkillsService {
  ...
  async create(CreateSkillRequest: CreateSkillDto): Promise<Skills> {
    ...
    const existingIdx = await this.skillsRepository.findOne({
      where: { index: CreateSkillRequest.index },
    });
    if (existingIdx) {
      //인덱스를 뒤로 미루는 로직
      const toUpdateIdx = await this.skillsRepository.find({
        where: [
          { index: MoreThan(CreateSkillRequest.index) },
          { index: CreateSkillRequest.index },
        ],
      });
  ...
  async delete(id: number): Promise<void> {
    ...
    const toUpdateIdx = await this.skillsRepository.find({
      where: [{ index: MoreThan(toDelete.index) }],
    });
    toUpdateIdx.forEach(async (skill) => {
      skill.index -= 1;
      await this.skillsRepository.save(skill);
  ...
  async update(
    id: number,
    UpdateSkillRequest: UpadateSkillDto,
  ): Promise<Skills> {
    const toUpdate = await this.skillsRepository.findOne({ where: { id: id } });
    const existingIdx = await this.skillsRepository.findOne({
      where: { index: UpdateSkillRequest.index },
    });
    if (!toUpdate) {
    ...
    if (existingIdx) {
      const toUpdateIdx = await this.skillsRepository.find({
        where: [
          { index: MoreThan(UpdateSkillRequest.index) },
          { index: UpdateSkillRequest.index },
        ],
      });
      toUpdateIdx.forEach(async (skill) => {
        skill.index += 1;
